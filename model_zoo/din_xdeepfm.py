...
class ExDeepFM(torch.nn.Module):
    ...
    def forward(self, Xi, Xv, bag_feat_Xi, bag_lenghts):
        ...
        for idx, bag_feat_size in enumerate(self.bag_value_cnts, 0):
            ...
            if self.use_din is False or (idx not in self.din_key_offsets):
                print("idx", idx)
                pool_emb = torch.sum(fm_bag_embs[:, start_idx:start_idx + bag_feat_size, :], dim=1, keepdim=True)
                # print("pool_emb_before", pool_emb)
                # print("ex_bag", ex_bag_lenghts[:, [idx]])
                pool_emb = pool_emb / ex_bag_lenghts[:, [idx]]
                # print("pool_emb", pool_emb)
        ...
        if self.use_lr:
            ...
            for idx, bag_feat_size in enumerate(self.bag_value_cnts, 0):
                start_idx = sum(self.bag_value_cnts[:idx])
                bag_fm_first_orders.append(
                    torch.sum(fm_first_order_bag[:, start_idx:start_idx + bag_feat_size], dim=1, keepdim=True))  # N*1
...
if __name__ == '__main__':
    ...
    Xv[:, 1] = 0.2
    Xi = Variable(torch.LongTensor([[0, 4, 8], [0, 4, 9], [0, 4, 9]]))
    #
    p_bag = 20
    bag_value_cnts = [2, 3, 3]
    # bag_value_cnts = [2, 1, 2]
    bag_feat_Xi = Variable(
        torch.LongTensor([[1, 2, 7, 8, 0, 10, 11, 12], [1, 2, 7, 8, 9, 10, 11, 14], [1, 2, 7, 8, 9, 10, 11, 15]]))
    # bag_feat_Xi = Variable(torch.LongTensor([[7, 8, 9, 0], [10, 11, 12, 13], [1, 2, 3, 4]]))
    bag_lenghts = Variable(torch.FloatTensor([[2, 3, 4], [5, 6, 7], [8, 9, 10]]))
    # bag_lenghts = Variable(torch.FloatTensor([[2, 2, 3]]))
    ...
    model = ExDeepFM(field_num, p_sv, p_bag, bag_value_cnts,
                     ...
                     din_query_offset=2, din_key_offsets=[0, 1, 2]
                     )

    # os.environ["CUDA_VISIBLE_DEVICES"] = '1'
    # model = model.cuda()
    # print(model(Xi.cuda(), Xv.cuda(), bag_feat_Xi.cuda(), bag_lenghts.cuda()))
    print(model(Xi, Xv, bag_feat_Xi, bag_lenghts))
